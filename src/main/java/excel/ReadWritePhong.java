...
public class ReadWritePhong {
	public static List<phong> ReadFileExcell(String file) {
		...
		try {
			...
			for (int i = 1; i < dem; i++) {
				...
				phong p = new phong(sh1.getRow(i).getCell(0),sh1.getRow(i).getCell(1),sh1.getRow(i).getCell(2),sh1.getRow(i).getCell(3),
						sh1.getRow(i).getCell(4),sh1.getRow(i).getCell(5),i);
				list.add(p);
			}
		} catch (IOException e) {
