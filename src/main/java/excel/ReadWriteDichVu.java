...
public class ReadWriteDichVu {
	
	
	public static List<dichvu> ReadFileExcell(String file) {
		...
		try {
			...
			for (int i = 1; i < dem; i++) {
				...
				dichvu u = new dichvu(sh1.getRow(i).getCell(0),sh1.getRow(i).getCell(1),sh1.getRow(i).getCell(2), i);
				list.add(u);
			}
		} catch (IOException e) {
